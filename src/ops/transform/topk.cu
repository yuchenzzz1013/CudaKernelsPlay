...
template <int K>
__device__ __forceinline__ void warp_reduce_topk(TopKRegister<K>& topk) {
...
template <int K>
__global__ void topk_kernel(const float* __restrict__ d_input,
                            float* __restrict__ d_output_value,
                            int* __restrict__ d_output_index, int rows,
                            int cols) {
  ...
  const float* row_ptr = d_input + (size_t)row * cols;
...
#pragma unroll
  ...
  if (tid < K) {
    const int out = row * K + tid;

    d_output_value[out] = key[0];

    d_output_index[out] = val[0];
  }
}
...
void TopK(const float* d_input, float* d_output_value, int* d_output_index,
  ...
  topk_kernel<K><<<grid, block, 0, stream>>>(d_input, d_output_value,
                                             d_output_index, rows, cols);

  CUDA_CHECK(cudaGetLastError());
}

// 显式实例化
template void TopK<10>(const float*, float*, int*, int, int, cudaStream_t);
